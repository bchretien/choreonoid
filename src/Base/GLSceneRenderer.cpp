...
void checkOpenGLError(const char* cmd, const char* fun, int line)
{
    ...
    {
        msg += (boost::format ("OpenGL error 0x%04x at %s:%i - for %s\n")
                % err % fun % line % cmd).str ();
    }
    if (!msg.empty())
        ...
        if(listID){
            if(isPicking && cache->useIDforPicking){
                if(!cache->listIDforPicking){
                    ...
                    if(cache->listIDforPicking){
                        GL_CHECK(glNewList(cache->listIDforPicking, GL_COMPILE));
                        isCompiling = true;
                        clearGLState();
                        self->visitGroup(group);
                        isCompiling = false;
